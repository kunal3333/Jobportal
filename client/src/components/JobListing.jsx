...
const JobListing = () => {
  ...
  // Fixing the state initialization for showFilter
  ...
  const handleLocationChange = (location) => {
    setSelectedLocations((prev) => prev.includes(location)
  ...
  useEffect(() => {
    ...
    const matchesLocation = (job) =>
      selectedLocations.length === 0 || selectedLocations.includes(job.location); // Fixed location case
    ...
    const newFilteredJobs = jobs
      ...
      .reverse() // Reverse the list if required
      .filter(
        (job) =>
          matchesCategory(job) &&
          matchesLocation(job) &&
      ...
      <section className="w-full lg:w-3/4 text-gray-800 max-lg:px-4">
        ...
        {filterJobs.length > 0 && (
          <div className="flex items-center justify-center space-x-2 mt-10">
            ...
            >
              <img className="w-5 h-5" src={assets.left_arrow_icon} alt="Previous Page" />
            </button>
            ...
            <button
              className="px-4 py-2 bg-gray-200 rounded-lg"
              onClick={() =>
                setCurrentPage(Math.min(currentPage + 1, Math.ceil(filterJobs.length / 6)))
              }
              disabled={currentPage === Math.ceil(filterJobs.length / 6)}>
              <img className="w-5 h-5" src={assets.right_arrow_icon} alt="Next Page" />
            </button>
          </div>
        )}
      </section>
